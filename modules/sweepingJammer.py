...
def main(top_block_cls=mpsk_stage1, options=None):
    ...
    def change_frequency():
        #cycle_time =   # Time to complete traversal of all frequencies
        #interval = 0.000625 # Should be minimum that hardware can perform without degredation / disorder
        #num_steps = math.floor(cycle_time / interva)
        #step = (2442e6 -22412e6) / num_steps
        #if lower bandwidth == more power, set bandwidth = step #is this variable_channel_width
        frequencies = [2412e6, 2462e6, 2422e6, 2437e6, 2452e6, 2417e6, 2457e6, 2427e6, 2447e6, 2432e6, 2442e6] 
        #Channels 1, 11, 3, 6, 9, 2, 10, 4, 8, 5, 7
        freq = 2412e6
        while True:
            tb.set_Center_Frequency(freq)
            time.sleep(interval)  # Adjust sleep time as needed
            if freq <= 2442e6:
                freq += step
            else:
                freq = 2412e6
