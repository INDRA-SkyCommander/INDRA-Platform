# Elijah - 2026-02-17
# I would like to repurpose the 'skyjack' module for MitM video interception attacks. 

# Theoretical current state:
# Deauthenticates a drone connection and takes control
# Sends a land command to safely ground the hijacked drone

# Ideas
# - After connecting to the wifi, maybe there is a way to rename the SSID? That way, the fake AP will have the same name as the real drone 
#   and the controller will automatically connect to it instead of the real drone. Then we can forward traffic from the fake AP to the 
#   real drone to maintain control and video feed while hijacked.
...
class Skyjack:
    def __init__(self):
        ...
        self.tello_video_port = 11111  # Video stream port
        self.local_port = 9000  # Local port to bind
        self.local_video_port = 11111  # Local video stream port?
        self.local_ip = None
    ...
    # May be easier to just run the deauth attack using the deauth module and then run the connect/init steps here.
    ...
    def connect_to_drone(self) -> bool:
        ...
        try:
            ...
            # If this cmd doesn't work, consider using the IBSS/ad-hoc mode
