...
Replaces the old iwlist-based parser: iwlist relies on the legacy Wireless
Extensions (WEXT) ioctl API, which out-of-tree drivers (e.g. rtl8812au,
used on the INDRA scan adapters) often implement poorly or not at all.
`iw` talks to the kernel over nl80211 directly, which is what those drivers
actually support, so it scans faster and more reliably.
"""
